...
def Graph_Bivariate(v_vector: np.array, orig_x: np.array, orig_y:np.array, orig_z: np.array):
    """
    ...
    for index in range(len(sample_x)):
        calculated_z.append(np.double(Bivariate(v_vector, sample_x[index],sample_y[index])))
    ...
    if len(sample_x) % 2 != 0:
        ...
        newZ = Bivariate(v_vector, newX, newY)
